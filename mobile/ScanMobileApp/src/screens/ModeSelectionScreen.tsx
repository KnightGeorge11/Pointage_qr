...
const ModeSelectionScreen = () => {
  ...
  const handleSelect = async (mode: 'normal' | 'garde') => {
    ...
    try {
      ...
      if (mode === 'garde') {
        if (scanInfo.garde_planifiee) isFirstScan = true;
        else if (scanInfo.garde_en_cours) isFirstScan = false;
      } else {
        isFirstScan = scanInfo.first_scan;
      }
      const result = await apiService.recordScan(employeeQr, siteId, mode, isFirstScan);
  ...
  const hasGardeAlert = scanInfo.garde_planifiee || scanInfo.garde_en_cours;
      ...
      <View style={styles.cardsContainer}>
        ...
        >
          ...
          <View style={styles.cardContent}>
            ...
            <Text style={styles.cardDesc}>Pointage standard · matin / après-midi</Text>
          </View>
          <Ionicons name="chevron-forward" size={20} color="#ccc" />
        </TouchableOpacity>

        <TouchableOpacity
          style={[styles.card, styles.gardeCard, loading && styles.cardDisabled]}
          onPress={() => handleSelect('garde')}
          disabled={loading}
          activeOpacity={0.7}
        >
          ...
          <View style={styles.cardContent}>
            <Text style={styles.cardTitle}>Garde de nuit</Text>
            <Text style={styles.cardDesc}>Service de garde · période nuit</Text>
          </View>
          <Ionicons name="chevron-forward" size={20} color="#ccc" />
        </TouchableOpacity>
      </View>
