    ...
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <View style={styles.employeeRow}>
          <Ionicons name="person-circle-outline" size={18} color={colors.inkMuted} />
      ...
      <View style={styles.cardsContainer}>
        <TouchableOpacity style={[styles.card, loading && styles.cardDisabled]} onPress={() => handleSelect('normal')} disabled={loading} activeOpacity={0.7}>
          <View style={[styles.iconWrapper, { backgroundColor: colors.surfaceAlt }]}><Ionicons name="time-outline" size={32} color={colors.ink} /></View>
          ...
          <Ionicons name="chevron-forward" size={20} color={colors.line} />
        </TouchableOpacity>
        <TouchableOpacity style={[styles.card, styles.gardeCard, loading && styles.cardDisabled]} onPress={() => handleSelect('garde')} disabled={loading} activeOpacity={0.7}>
          <View style={[styles.iconWrapper, { backgroundColor: colors.amberDim }]}><Ionicons name="shield-checkmark-outline" size={32} color={colors.amberText} /></View>
      ...
      {loading && <View style={styles.loadingOverlay}><ActivityIndicator size="large" color={colors.ink} /><Text style={styles.loadingText}>Enregistrement...</Text></View>}
    </SafeAreaView>
  );
};
...
const styles = StyleSheet.create({
  ...
  employeeName: { fontSize: 15, color: colors.inkMuted, fontWeight: '500' },
  alertBanner: { flexDirection: 'row', alignItems: 'flex-start', marginHorizontal: 20, marginBottom: 20, padding: 14, backgroundColor: colors.amberDim, borderRadius: radius.md, gap: 10 },
  alertText: { flex: 1, fontSize: 13, color: colors.amberText, lineHeight: 18 },
  cardsContainer: { paddingHorizontal: 20, gap: 12 },
  card: { flexDirection: 'row', alignItems: 'center', backgroundColor: colors.white, borderRadius: radius.lg, padding: 16, shadowColor: '#000', shadowOpacity: 0.04, shadowOffset: { width: 0, height: 2 }, shadowRadius: 6, elevation: 2, gap: 14 },
  gardeCard: { borderWidth: 1.5, borderColor: colors.amber },
  cardDisabled: { opacity: 0.5 },
  iconWrapper: { width: 56, height: 56, borderRadius: radius.lg, justifyContent: 'center', alignItems: 'center' },
  cardContent: { flex: 1 },
  cardTitle: { fontSize: 17, fontWeight: '600', color: colors.ink, marginBottom: 3 },
  cardDesc: { fontSize: 13, color: colors.inkMuted },
  ...
  loadingText: { fontSize: 15, color: colors.inkSoft, fontWeight: '500' },
});
