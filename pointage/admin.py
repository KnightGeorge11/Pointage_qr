    ...
    class PointageAdmin(admin.ModelAdmin):
        ...
        def exporter_excel(self, request, queryset):
            """
            Action admin : Export Excel des pointages sélectionnés
            """
            import openpyxl
            from openpyxl.styles import Font, PatternFill, Alignment, Border, Side
            from openpyxl.utils import get_column_letter
            ...
            # Récupérer les dates min et max pour le titre
            dates = queryset.values_list('date_pointage', flat=True).distinct().order_by('date_pointage')
            if not dates:
                self.message_user(request, "Aucun pointage sélectionné.", level='warning')
                return
            ...
            qs = queryset.select_related('employe', 'site', 'employe__poste').order_by(
            ...
            for emp_idx, (emp_id, day_map) in enumerate(emp_data.items()):
                ...
                for i, d in enumerate(days):
                    ...
                    if nuit and nuit.type_journee == 'garde':
                        ...
                        sc(ws.cell(row=base, column=col), bg=NIGHT_BG,
                        ...
                        value='Début  →  Fin',
                        bg=NIGHT_BG, fg=NIGHT_FG, bold=True, size=8,
            ...
            self.message_user(request, f"✅ Export Excel généré avec succès pour {len(emp_data)} employés.")
            return response
        
        exporter_excel.short_description = "📊 Exporter vers Excel"
